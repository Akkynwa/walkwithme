...
function OnboardingWizardFlow() {
  ...
  const [moodPayload, setMoodPayload] = useState<MoodPayload>({
    selectedMood: '',
    suggestScriptures: true,
    notes: ''
  });

  // Advance automatically if an active session exists on initial load
  useEffect(() => {
  ...
  const handleMoodSuccess = (payload: MoodPayload) => {
  ...
  return (
    <div className="relative min-h-screen w-full flex items-center justify-center p-4 sm:p-6 bg-zinc-100 font-sans">
      ...
      {/* BACKGROUND CONTENT LAYER (Simulates parent workspace behind modal view) */}
      <div className="absolute inset-0 z-0 opacity-40 pointer-events-none filter blur-sm flex flex-col p-8 justify-between">
        <div className="flex justify-between items-center w-full">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 bg-zinc-400 rounded-lg" />
        ...
        <div className="grid grid-cols-3 gap-6 my-auto max-w-4xl w-full mx-auto">
          <div className="h-32 bg-zinc-300 rounded-xl" />
          <div className="h-32 bg-zinc-300 rounded-xl" />
          <div className="h-32 bg-zinc-300 rounded-xl" />
        </div>
      </div>
      ...
      <div className="relative z-20 w-full max-w-[920px] overflow-hidden rounded-[32px] border border-orange-100 bg-white/90 shadow-[0_20px_80px_-30px_rgba(15,23,42,0.35)] backdrop-blur-xl md:flex min-h-[560px]">
        ...
        <Link 
          href="/"
          className="absolute right-4 top-4 z-30 flex items-center justify-center rounded-full bg-orange-50 p-2 text-orange-600 transition hover:bg-orange-100"
          aria-label="Close form view"
        >
          <span className="material-symbols-outlined text-xl">close</span>
        </Link>

        {/* LEFT COMPONENT COLUMN: Visual Hero Asset */}
        <div className="w-full md:w-1/2 bg-orange-50/70 p-8 flex items-center justify-center border-r border-orange-100 select-none">
          <Image
            src="https://images.unsplash.com/photo-1653569746987-8c1c63b2ffe2?q=80&w=870&auto=format&fit=crop&ixlib=rb-4.1.0"
            alt="Ebook Showcase Guide"
            width={800}
            height={600}
            priority
        ...
        <div className="w-full md:w-1/2 p-8 sm:p-12 flex flex-col justify-center bg-white">
          <Suspense fallback={
            <div className="w-full flex flex-col items-center justify-center py-20">
              <p className="text-xs font-bold uppercase tracking-widest text-zinc-900 animate-pulse">
                Mounting engine components...
              </p>
            </div>
          }>
            {currentStep === 1 && (
            ...
            {currentStep === 4 && (
              <MeditationStep 
                moodContext={moodPayload} 
                onBack={() => setCurrentStep(3)} 
                onComplete={handleFinalizePipeline} 
              />
            )}
          </Suspense>
        </div>
